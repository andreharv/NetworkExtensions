		...
		*/
		...
		private void PathFindImplementation(uint unit, ref PathUnit data) {
			...
			for (int k = 0; k < 262144; k++) {
				...
				CustomRoadAI.AddTraffic(laneID, (ushort)(this._isHeavyVehicle ? 3 : 1), (ushort)SpeedLimitManager.GetLockFreeGameSpeedLimit(laneID), false);
				// NON-STOCK CODE END
...
#region stock code
		...
		private void ProcessItem2(BufferItem item, ushort targetNodeId, bool targetDisabled, ushort nextSegmentId, ref NetSegment nextSegment, uint lane, byte offset, byte connectOffset) {
			...
			if ((int)item.m_position.m_lane < prevSegmentInfo.m_lanes.Length) {
				...
				num3 = SpeedLimitManager.GetLockFreeGameSpeedLimit(item.m_laneID); // NON-STOCK CODE
				laneType = lane2.m_laneType;
				if ((byte)(laneType & (NetInfo.LaneType.Vehicle | NetInfo.LaneType.TransportVehicle)) != 0) {
					laneType = (NetInfo.LaneType.Vehicle | NetInfo.LaneType.TransportVehicle);
				}
				num4 = this.CalculateLaneSpeed(item.m_laneID, connectOffset, item.m_position.m_offset, ref instance.m_segments.m_buffer[(int)item.m_position.m_segment], lane2); // NON-STOCK CODE
			}
				...
				if (lane == curLaneId) {
					...
					if (lane3.CheckType(this._laneTypes, this._vehicleTypes)) {
						...
						if (lane3.m_laneType != NetInfo.LaneType.Pedestrian || item2.m_methodDistance < 1000f) {
							item2.m_comparisonValue = num7 + num9 / ((num3 + SpeedLimitManager.GetLockFreeGameSpeedLimit(curLaneId)) * 0.5f * this._maxLength); // NON-STOCK CODE
							...
							if (lane == this._startLaneA) {
								...
								float num10 = this.CalculateLaneSpeed(curLaneId, this._startOffsetA, item2.m_position.m_offset, ref nextSegment, lane3); // NON-STOCK CODE
							...
							if (lane == this._startLaneB) {
								...
								float num12 = this.CalculateLaneSpeed(curLaneId, this._startOffsetB, item2.m_position.m_offset, ref nextSegment, lane3); // NON-STOCK CODE
		...
		private bool ProcessItemSub(bool allowCustomLaneChanging, bool debug, BufferItem item, ushort targetNodeId, ushort segmentID, ref NetSegment nextSegment, ref int laneIndexFromLeft, byte connectOffset, bool enableVehicle, bool enablePedestrian, int? forceLaneIndex, uint? forceLaneId, out bool foundForced) {
			...
			if ((int)item.m_position.m_lane < prevSegmentInfo.m_lanes.Length) {
				...
				prevMaxSpeed = SpeedLimitManager.GetLockFreeGameSpeedLimit(item.m_laneID); // NON-STOCK CODE
				prevLaneSpeed = this.CalculateLaneSpeed(item.m_laneID, connectOffset, item.m_position.m_offset, ref instance.m_segments.m_buffer[(int)item.m_position.m_segment], lane); // NON-STOCK CODE
				// NON-STOCK CODE START //
			...
			while (laneIndex < nextNumLanes && curLaneId != 0u) {
				...
				float nextMaxSpeed = SpeedLimitManager.GetLockFreeGameSpeedLimit(curLaneId); // NON-STOCK CODE
				// NON-STOCK CODE START //
				if (customLaneChanging) {
					...
					if (nextLane.CheckType(laneType2, vehicleType2) && (segmentID != item.m_position.m_segment || laneIndex != (int)item.m_position.m_lane) && (byte)(nextLane.m_finalDirection & nextDir2) != 0) {
						...
						if (nextLane.m_laneType != NetInfo.LaneType.Pedestrian || item2.m_methodDistance < 1000f) {
							...
							if (curLaneId == this._startLaneA) {
								...
								float num15 = this.CalculateLaneSpeed(curLaneId, this._startOffsetA, item2.m_position.m_offset, ref nextSegment, nextLane); // NON-STOCK CODE
							...
							if (curLaneId == this._startLaneB) {
								...
								float num17 = this.CalculateLaneSpeed(curLaneId, this._startOffsetB, item2.m_position.m_offset, ref nextSegment, nextLane); // NON-STOCK CODE
...
#region stock code
		...
		private void ProcessItemPedBicycle(BufferItem item, ushort targetNodeId, ushort segmentID, ref NetSegment segment, byte connectOffset, int laneIndex, uint lane) {
			...
			if (laneIndex < num) {
				...
				if (lane3.m_laneType != NetInfo.LaneType.Pedestrian || item2.m_methodDistance < 1000f) {
					item2.m_comparisonValue = num7 + num2 / ((num3 + SpeedLimitManager.GetLockFreeGameSpeedLimit(lane)) * 0.25f * this._maxLength); // NON-STOCK CODE
					...
					if (lane == this._startLaneA) {
						...
						float num8 = this.CalculateLaneSpeed(lane, this._startOffsetA, item2.m_position.m_offset, ref segment, lane3); // NON-STOCK CODE
					...
					if (lane == this._startLaneB) {
						...
						float num10 = this.CalculateLaneSpeed(lane, this._startOffsetB, item2.m_position.m_offset, ref segment, lane3); // NON-STOCK CODE
		...
		private float CalculateLaneSpeed(uint laneId, byte startOffset, byte endOffset, ref NetSegment segment, NetInfo.Lane laneInfo) {
			float speedLimit = SpeedLimitManager.GetLockFreeGameSpeedLimit(laneId);
