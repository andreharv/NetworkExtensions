...
namespace TransitPlus.Addon.RoadExtensions
{
    ...
    {
        [UsedImplicitly]
        private class RoadsInstaller : Installer<RExPlusModule>
        {
            ...
            protected override void Install(RExPlusModule host)
            {
                ...
                swAll.Stop();
                Debug.Log($"All RExPlusModule NetInfos in {swAll.ElapsedMilliseconds}ms");
                InstallNetInfosModifiers(host);
                InstallCompatibilities(host);
            }
            private static void InstallPropInfos(RExPlusModule host)
            {
                ...
                foreach (var piBuilder in piBuilders)
                {
                    ...
                    Loading.QueueAction(() =>
                    {
                        try
                        {
                            ...
                            sw.Stop();
                            Debug.Log(string.Format("RExPlus Prop {0} installed in {1}ms", builder.Name, sw.ElapsedMilliseconds));
                        }
                        catch (Exception ex)
                        {
                            Debug.Log(string.Format("RExPlus Crashed-Prop builder {0}", builder.Name));
                            Debug.Log("RExPlus " + ex.Message);
                            Debug.Log("RExPlus " + ex.ToString());

                            Debug.Log(string.Format("RExPlus Fallbacking-Prop builder {0}", builder.Name));
                            try
                            {
                            ...
                            {
                                Debug.Log(string.Format("RExPlus Crashed-Fallback Prop builder {0}", builder.Name));
                                Debug.Log("RExPlus " + exFallback.Message);
                                Debug.Log("RExPlus " + exFallback.ToString());
                            }
                        }
                    });
                }
            ...
            private void InstallNetInfos(RExPlusModule host)
            {
                ...
                foreach (var niBuilder in niBuilders)
                {
                    ...
                    Loading.QueueAction(() =>
                    {
                        ...
                        try
                        {
                            ...
                            Debug.Log(string.Format("RExPlus {0} installed in {1}ms", builder.Name, sw.ElapsedMilliseconds));
                        }
                        catch (Exception ex)
                        {

                            Debug.Log(string.Format("RExPlus Crashed-Network builder {0}", builder.Name));
                            Debug.Log("RExPlus " + ex.Message);
                            Debug.Log("RExPlus " + ex.ToString());

                            Debug.Log(string.Format("RExPlus Fallbacking-Network builder {0}", builder.Name));
                            try
                            {
                            ...
                            {
                                Debug.Log(string.Format("RExPlus Crashed-Fallback Network builder {0}", builder.Name));
                                Debug.Log("RExPlus " + exFallback.Message);
                                Debug.Log("RExPlus " + exFallback.ToString());
                            }
                        }
            ...
            private static void InstallNetInfosModifiers(RExPlusModule host)
            {
                ...
                foreach (var nimodifier in niModifiers)
                {
                    ...
                    Loading.QueueAction(() =>
                    {
                        try
                        {
                            ...
                            sw.Stop();
                            Debug.Log(string.Format("RExPlus {0} modifications applied in {1}ms", modifier.Name, sw.ElapsedMilliseconds));
                        }
                        catch (Exception ex)
                        {
                            Debug.Log(string.Format("RExPlus Crashed-Network modifiers {0}", modifier.Name));
                            Debug.Log("RExPlus " + ex.Message);
                            Debug.Log("RExPlus " + ex.ToString());
                        }
                    });
            ...
            private static void InstallCompatibilities(RExPlusModule host)
            {
                var swAll = new Stopwatch();
                swAll.Start();
                Stopwatch sw = new Stopwatch();
                    ...
                    foreach (var compatibilityPart in compParts)
                    {
                        ...
                        try
                        {
                            ...
                            if (compatibilityPart.IsPluginActive)
                            {
                                ...
                                sw2.Stop();
                                Debug.Log(string.Format("RExPlus {0} compatibility activated in {1}", compatibilityPart.Name, sw2.ElapsedMilliseconds));
                            }
                            else
                            {
                                sw2.Stop();
                        ...
                        {
                            Debug.Log(string.Format("RExPlus Crashed-CompatibilitySupport {0}", compatibilityPart.Name));
                            Debug.Log("RExPlus " + ex.Message);
                            Debug.Log("RExPlus " + ex.ToString());
                        }
                    }
