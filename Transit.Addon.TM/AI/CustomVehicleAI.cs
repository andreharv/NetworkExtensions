...
namespace Transit.Addon.TM.AI {
	class CustomVehicleAI : VehicleAI {
		...
		internal static void HandleVehicle(ushort vehicleId, ref Vehicle vehicleData, bool addTraffic, bool realTraffic, byte maxUpcomingPathPositions, bool debug = false) {
			...
			TMVehicleType? vehicleType = CustomVehicleAI.DetermineVehicleTypeFromVehicle(vehicleId, ref vehicleData);
			if (vehicleType == null) {
				Log.Warning($"Could not determine vehicle type of vehicle {vehicleId}!");
			}

			if (vehicleType == null || vehicleType == TMVehicleType.None) {
				return;
			}
		...
		protected virtual void CalculateSegPos(ushort vehicleID, ref Vehicle vehicleData, PathUnit.Position position, uint laneID, byte offset, out Vector3 pos, out Vector3 dir, out float maxSpeed) {
			...
			if (info.m_lanes != null && info.m_lanes.Length > (int)position.m_lane) {
				var laneSpeedLimit = TAMSpeedLimitManager.instance.GetLaneSpeedLimit(position.m_segment, position.m_lane, laneID, info.m_lanes[position.m_lane]);
		...
		internal static TMVehicleType? DetermineVehicleTypeFromAIType(VehicleAI ai, bool emergencyOnDuty) {
			if (emergencyOnDuty)
				return TMVehicleType.Emergency;

			switch (ai.m_info.m_vehicleType) {
				case VehicleInfo.VehicleType.Bicycle:
					return TMVehicleType.Bicycle;
				case VehicleInfo.VehicleType.Car:
					if (ai is PassengerCarAI)
						return TMVehicleType.PassengerCar;
					if (ai is AmbulanceAI || ai is FireTruckAI || ai is PoliceCarAI || ai is HearseAI || ai is GarbageTruckAI || ai is MaintenanceTruckAI || ai is SnowTruckAI) {
						return TMVehicleType.Service;
					}
					if (ai is CarTrailerAI)
						return TMVehicleType.None;
					if (ai is BusAI)
						return TMVehicleType.Bus;
					if (ai is TaxiAI)
						return TMVehicleType.Taxi;
					if (ai is CargoTruckAI)
						return TMVehicleType.CargoTruck;
					break;
				...
				case VehicleInfo.VehicleType.Ship:
					if (ai is PassengerShipAI)
						return TMVehicleType.PassengerShip;
					//if (ai is CargoShipAI)
						return TMVehicleType.CargoShip;
					//break;
				case VehicleInfo.VehicleType.Plane:
					//if (ai is PassengerPlaneAI)
						return TMVehicleType.PassengerPlane;
					//break;
			}
