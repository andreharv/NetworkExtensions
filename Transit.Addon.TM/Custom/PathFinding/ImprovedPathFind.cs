...
namespace TrafficManager.Custom.PathFinding
{
    public class ImprovedPathFind : IPathFind
    {
        ...
        public bool CalculatePath(ExtendedUnitType vehicleType, uint unit, bool skipQueue)
        {
            if (Singleton<PathManager>.instance.AddPathReference(unit))
            {
                try
                {
                    ...
                    _pathUnitTypes[unit] = vehicleType;
        ...
        protected void PathFindImplementation(uint unit, ref PathUnit data)
        {
            ...
            if (!_pathUnitTypes.TryGetValue(unit, out _unitType))
            {
                this._unitType = ExtendedUnitType.Unknown;
            }
...
#endif
                _pathUnitTypes[unit] = ExtendedUnitType.Unknown;

                /*PathUnit[] expr_909_cp_0 = _pathUnits.m_buffer;
        ...
        private bool ProcessItemCosts(bool allowCustomLaneChanging, bool debug, BufferItem item, ushort targetNodeId, ushort nextSegmentId, ref NetSegment nextSegment, ref int laneIndexFromLeft, byte connectOffset, bool enableVehicle, bool enablePedestrian, int? forceLaneIndex, uint? forceLaneId, out bool foundForced)
        {
            ...
            if ((int)item.m_position.m_lane < prevSegmentInfo.m_lanes.Length)
            {
                ...
                prevMaxSpeed = this.GetFeatures().RoadSpeed.GetLaneSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, lane, _unitType);  // NON-STOCK CODE
                ...
                prevLaneSpeed = CalculateLaneSpeed(prevMaxSpeed, connectOffset, item.m_position.m_offset, ref instance.m_segments.m_buffer[(int)item.m_position.m_segment], lane); // NON-STOCK CODE
                                                                                                                                                                                        // NON-STOCK CODE START //
            ...
            if (!this.GetFeatures().RoadRestriction.CanUseLane(item.m_laneID, item.m_position.m_segment, item.m_position.m_lane, _unitType))
            {
                //if (!CanUseLane(debug, item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, lane)) {
                if (Options.disableSomething1 && debug)
                {
                    Log._Debug($"Vehicle {_unitType} must not use lane {item.m_position.m_lane} @ seg. {item.m_position.m_segment}, null? {lane == null}");
                }
            ...
            while (laneIndex < nextNumLanes && curLaneId != 0u)
            {
                ...
                if ((byte)(nextLane.m_finalDirection & nextFinalDir) != 0 &&
                    this.GetFeatures().LaneRouting.CanLanesConnect(targetNodeId, curLaneId, item.m_laneID, _unitType))
                {
