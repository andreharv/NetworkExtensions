...
namespace Transit.Addon.TM.PathFinding
{
	public class TAMImprovedPathFind : IPathFind
	{
		...
		private uint _queueFirst;
		private uint _queueLast;
		private uint _calculating;
		private object _queueLock;
		private Thread _pathFindThread;
		...
		private static ExtendedUnitType?[] _pathUnitTypes = new ExtendedUnitType?[PathManager.MAX_PATHUNIT_COUNT];
		...
		private byte[] _laneIndexes = new byte[16]; // index of NetSegment.Info.m_lanes
		private uint[] _laneIds = new uint[16]; // index of NetManager.m_lanes.m_buffer
		private byte[] _laneRightSimilarIndexes = new byte[16];
		private byte[] _laneLeftSimilarIndexes = new byte[16];
		private byte[] _laneIndexByRightSimilarIndex = new byte[16];
		private ushort _compatibleRightSimilarIndexesMask = (ushort)0;
		private static readonly ushort[] _pow2Masks = { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768 };
		...
		public void OnAwake()
		{
			...
			_bufferLock = Singleton<PathManager>.instance.m_bufferLock;
			_pathUnits = Singleton<PathManager>.instance.m_pathUnits;
			_pathFindThread = new Thread(new ThreadStart(PathFindThread));
			_pathFindThread.Name = "Pathfind";
			_pathFindThread.Priority = SimulationManager.SIMULATION_PRIORITY;
			_pathFindThread.Start();
		}
				...
				try
				{
					...
					if (skipQueue)
					{
						if (_queueLast == 0u)
						{
							...
							_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_nextPathUnit = _queueFirst;
						}
						_queueFirst = unit;
					}
					else {
						if (_queueLast == 0u)
						{
							...
							_pathUnits.m_buffer[(int)((UIntPtr)_queueLast)].m_nextPathUnit = unit;
						}
						_queueLast = unit;
					}
					_pathUnits.m_buffer[unit].m_pathFindFlags |= PathUnit.FLAG_CREATED;
					Facade.m_queuedPathFindCount++;
					_pathUnitTypes[unit] = unitType;
		...
		{
			while (!Monitor.TryEnter(_queueLock, SimulationManager.SYNCHRONIZE_TIMEOUT))
			{
			}
			try
			{
				while ((_queueFirst != 0u || _calculating != 0u) && !_terminated)
				{
		...
		protected void PathFindImplementation(uint unit, ref PathUnit data)
		{
			...
			_laneTypes = (NetInfo.LaneType)_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_laneTypes;
			_vehicleTypes = (VehicleInfo.VehicleType)_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_vehicleTypes;
			_maxLength = _pathUnits.m_buffer[(int)((UIntPtr)unit)].m_length;
			_pathFindIndex = (_pathFindIndex + 1u & 32767u);
			_pathRandomizer = new Randomizer(unit);
			_isHeavyVehicle = ((_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_simulationFlags & 16) != 0);
			_ignoreBlocked = ((_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_simulationFlags & 32) != 0);
			_stablePath = ((_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_simulationFlags & 64) != 0);
			_transportVehicle = ((byte)(_laneTypes & NetInfo.LaneType.TransportVehicle) != 0);
			if ((byte)(_laneTypes & NetInfo.LaneType.Vehicle) != 0)
			{
				_laneTypes |= NetInfo.LaneType.TransportVehicle;
			}

			ExtendedUnitType? unitType = _pathUnitTypes[unit];
			if (unitType != null)
				_unitType = (ExtendedUnitType)unitType;
			else
				_unitType = ExtendedUnitType.Unknown;

			int num = (int)(_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_positionCount & 15);
			int num2 = _pathUnits.m_buffer[(int)((UIntPtr)unit)].m_positionCount >> 4;
			...
			if (data.m_position00.m_segment != 0 && num >= 1)
			{
				...
				bufferItemStartA.m_laneID = _startLaneA;
			...
			if (data.m_position02.m_segment != 0 && num >= 3)
			{
				...
				bufferItemStartB.m_laneID = _startLaneB;
			...
			if (data.m_position01.m_segment != 0 && num >= 2)
			{
				...
				bufferItemEndA.m_laneID = _endLaneA;
			...
			if (data.m_position03.m_segment != 0 && num >= 4)
			{
				...
				bufferItemEndB.m_laneID = _endLaneB;
			...
			{
				_bufferMax[0]++;
				_buffer[++_bufferMaxPos] = bufferItemEndA;
			}
			if (bufferItemEndB.m_position.m_segment != 0)
			{
				_bufferMax[0]++;
				_buffer[++_bufferMaxPos] = bufferItemEndB;
			}
			bool canFindPath = false;
#if DEBUGPF
			uint wIter = 0;
#endif
			//Log.Message($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: STARTING MAIN LOOP! bufferMinPos: {_bufferMinPos}, bufferMaxPos: {_bufferMaxPos}, startA: {bufferItemStartA.m_position.m_segment}, startB: {bufferItemStartB.m_position.m_segment}, endA: {bufferItemEndA.m_position.m_segment}, endB: {bufferItemEndB.m_position.m_segment}");
			while (_bufferMinPos <= _bufferMaxPos)
			{
				//pfCurrentState = 1;
#if DEBUGPF
				/*if (m_queuedPathFindCount > 100 && Options.disableSomething1)
					Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: MAIN LOOP RUNNING! bufferMinPos: {_bufferMinPos}, bufferMaxPos: {_bufferMaxPos}, startA: {bufferItemStartA.m_position.m_segment}, startB: {bufferItemStartB.m_position.m_segment}, endA: {bufferItemEndA.m_position.m_segment}, endB: {bufferItemEndB.m_position.m_segment}");*/
#endif
				int bufMin = _bufferMin[_bufferMinPos];
				int bufMax = _bufferMax[_bufferMinPos];
				if (bufMin > bufMax)
				{
					_bufferMinPos++;
				}
				else {
					_bufferMin[_bufferMinPos] = bufMin + 1;
					BufferItem candidateItem = _buffer[(_bufferMinPos << 6) + bufMin];
					...
					{
						// we reached startA
						if ((byte)(candidateItem.m_direction & NetInfo.Direction.Forward) != 0 && candidateItem.m_position.m_offset >= _startOffsetA)
						{
							finalBufferItem = candidateItem;
							startOffset = _startOffsetA;
							canFindPath = true;
							break;
						}
						if ((byte)(candidateItem.m_direction & NetInfo.Direction.Backward) != 0 && candidateItem.m_position.m_offset <= _startOffsetA)
						{
							finalBufferItem = candidateItem;
							startOffset = _startOffsetA;
							canFindPath = true;
					...
					{
						// we reached startB
						if ((byte)(candidateItem.m_direction & NetInfo.Direction.Forward) != 0 && candidateItem.m_position.m_offset >= _startOffsetB)
						{
							finalBufferItem = candidateItem;
							startOffset = _startOffsetB;
							canFindPath = true;
							break;
						}
						if ((byte)(candidateItem.m_direction & NetInfo.Direction.Backward) != 0 && candidateItem.m_position.m_offset <= _startOffsetB)
						{
							finalBufferItem = candidateItem;
							startOffset = _startOffsetB;
							canFindPath = true;
					...
					if ((byte)(candidateItem.m_direction & NetInfo.Direction.Forward) != 0)
					{
						...
						ProcessItemMain(unit, candidateItem, startNode, ref instance.m_nodes.m_buffer[(int)startNode], 0, false);
					}
					...
					if ((byte)(candidateItem.m_direction & NetInfo.Direction.Backward) != 0)
					{
						...
						ProcessItemMain(unit, candidateItem, endNode, ref instance.m_nodes.m_buffer[(int)endNode], 255, false);
					}
					...
					if (specialNodeId != 0)
					{
						...
						while (specialNodeId != 0)
						{
							...
							{
								ProcessItemMain(unit, candidateItem, specialNodeId, ref instance.m_nodes.m_buffer[(int)specialNodeId], laneOffset, true);
							}
			...
			/*if (m_queuedPathFindCount > 100 && Options.disableSomething0)
				Log.Message($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: MAIN LOOP FINISHED! bufferMinPos: {_bufferMinPos}, bufferMaxPos: {_bufferMaxPos}, startA: {bufferItemStartA.m_position.m_segment}, startB: {bufferItemStartB.m_position.m_segment}, endA: {bufferItemEndA.m_position.m_segment}, endB: {bufferItemEndB.m_position.m_segment}");*/
			if (!canFindPath)
			{
				// we could not find a path
				_pathUnits.m_buffer[(int)unit].m_pathFindFlags |= PathUnit.FLAG_FAILED;
#if DEBUG
				++_failedPathFinds;
				//Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: Cannot find path (pfCurrentState={pfCurrentState}) for unit {unit}");
#endif
				_pathUnitTypes[unit] = null;
				return;
			}
			// we could calculate a valid path

			float totalPathLength = finalBufferItem.m_comparisonValue * _maxLength;
			_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_length = totalPathLength;
				...
				(currentPosition.m_segment != bufferItemEndB.m_position.m_segment || currentPosition.m_lane != bufferItemEndB.m_position.m_lane || currentPosition.m_offset != bufferItemEndB.m_position.m_offset))
			{
				...
				if (startOffset != currentPosition.m_offset)
				{
					...
					_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].SetPosition(currentItemPositionCount++, position2);
					// now we have: [desired starting position]
				}
				// add the found starting position to the path unit
				_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].SetPosition(currentItemPositionCount++, currentPosition);
				currentPosition = _laneTarget[(int)((UIntPtr)finalBufferItem.m_laneID)]; // go to the next path position
			...
			{
				//pfCurrentState = 6;
				_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].SetPosition(currentItemPositionCount++, currentPosition); // add the next path position to the current unit
					...
					(currentPosition.m_segment == bufferItemEndB.m_position.m_segment && currentPosition.m_lane == bufferItemEndB.m_position.m_lane && currentPosition.m_offset == bufferItemEndB.m_position.m_offset))
				{
					...
					_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].m_positionCount = (byte)currentItemPositionCount;
					...
					if (sumOfPositionCounts != 0)
					{
						...
						currentPathUnitId = _pathUnits.m_buffer[(int)((UIntPtr)unit)].m_nextPathUnit; // (we do not need to calculate the length for the starting unit since this is done before; it's the total path length)
						currentItemPositionCount = (int)_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_positionCount;
						int totalIter = 0;
						while (currentPathUnitId != 0u)
						{
							_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].m_length = totalPathLength * (float)(sumOfPositionCounts - currentItemPositionCount) / (float)sumOfPositionCounts;
							currentItemPositionCount += (int)_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].m_positionCount;
							currentPathUnitId = _pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].m_nextPathUnit;
							if (++totalIter >= 262144)
							{
#if DEBUG
								Log.Error("THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: PathFindImplementation: Invalid list detected.");
#endif
...
#if DEBUG
					//Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: Path found (pfCurrentState={pfCurrentState}) for unit {unit}");
#endif
					_pathUnits.m_buffer[(int)unit].m_pathFindFlags |= PathUnit.FLAG_READY; // Path found
#if DEBUG
					++_succeededPathFinds;
#endif
					_pathUnitTypes[unit] = null;
				...
				if (currentItemPositionCount == 12)
				{
					...
					try
					{
						...
						if (!_pathUnits.CreateItem(out createdPathUnitId, ref _pathRandomizer))
						{
							// we failed to create a new path unit, thus the path-finding also failed
							_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_pathFindFlags |= PathUnit.FLAG_FAILED;
#if DEBUG
							++_failedPathFinds;
							//Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: Cannot find path (pfCurrentState={pfCurrentState}) for unit {unit}");
#endif
						...
						_pathUnits.m_buffer[(int)((UIntPtr)createdPathUnitId)] = _pathUnits.m_buffer[(int)currentPathUnitId];
						_pathUnits.m_buffer[(int)((UIntPtr)createdPathUnitId)].m_referenceCount = 1;
						_pathUnits.m_buffer[(int)((UIntPtr)createdPathUnitId)].m_pathFindFlags = 4;
						_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].m_nextPathUnit = createdPathUnitId;
						_pathUnits.m_buffer[(int)((UIntPtr)currentPathUnitId)].m_positionCount = (byte)currentItemPositionCount;
						sumOfPositionCounts += currentItemPositionCount;
						Singleton<PathManager>.instance.m_pathUnitCount = (int)(_pathUnits.ItemCount() - 1u);
					}
					catch (Exception e)
					{
				...
				CustomRoadAI.AddTraffic(
					...
					null,
					false);
				//TAMSpeedLimitManager.instance.GetLockFreeGameSpeedLimit(currentPosition.m_segment, currentPosition.m_lane, laneID, ref Singleton<NetManager>.instance.m_segments.m_buffer[currentPosition.m_segment].Info.m_lanes[currentPosition.m_lane])
				// NON-STOCK CODE END
				currentPosition = _laneTarget[(int)((UIntPtr)laneID)];
			}
			_pathUnits.m_buffer[(int)unit].m_pathFindFlags |= PathUnit.FLAG_FAILED;
#if DEBUG
			++_failedPathFinds;
		...
		private void ProcessItemMain(uint unitId, BufferItem item, ushort nextNodeId, ref NetNode nextNode, byte connectOffset, bool isMiddle)
		{
...
#if DEBUGPF
			/*if (m_queuedPathFindCount > 100 && Options.disableSomething1)
				Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: processItemMain RUNNING! item: {item.m_position.m_segment}, {item.m_position.m_lane} nextNodeId: {nextNodeId}");*/
#endif
			//Log.Message($"THREAD #{Thread.CurrentThread.ManagedThreadId} Path finder: " + _pathFindIndex + " vehicle types: " + _vehicleTypes);
#if DEBUGPF
...
#if DEBUGPF2
			bool debug2 = Options.disableSomething1 && _unitType == ExtendedUnitType.Bicycle;
			...
			if ((int)item.m_position.m_lane < prevSegmentInfo.m_lanes.Length)
			{
				...
				isBicycleLane = (prevLane.m_laneType == NetInfo.LaneType.Vehicle && (prevLane.m_vehicleType & _vehicleTypes) == VehicleInfo.VehicleType.Bicycle);
			...
			if (isMiddle)
			{
				for (int i = 0; i < 8; i++)
				{
					...
					if (nextSegmentId <= 0)
						continue;
					ProcessItemCosts(false, debug, item, nextNodeId, nextSegmentId, ref netManager.m_segments.m_buffer[(int)nextSegmentId], ref similarLaneIndexFromLeft, connectOffset, !isPedestrianLane, isPedestrianLane);
				}
			}
			else if (isPedestrianLane)
			{
...
#endif
						ProcessItemPedBicycle(item, nextNodeId, nextLeftSegment, ref netManager.m_segments.m_buffer[(int)nextLeftSegment], connectOffset, leftLaneIndex, leftLaneId); // ped
					}
...
#endif
						ProcessItemPedBicycle(item, nextNodeId, nextRightSegment, ref netManager.m_segments.m_buffer[(int)nextRightSegment], connectOffset, rightLaneIndex, rightLaneId); // ped
					}
					int nextLaneIndex;
					uint nextLaneId;
					if ((_vehicleTypes & VehicleInfo.VehicleType.Bicycle) != VehicleInfo.VehicleType.None &&
...
#endif
						ProcessItemPedBicycle(item, nextNodeId, prevSegmentId, ref netManager.m_segments.m_buffer[(int)prevSegmentId], connectOffset, nextLaneIndex, nextLaneId); // bicycle
					}
				}
				else {
					//mCurrentState = 9;
...
#endif
							ProcessItemCosts(false, debug, item, nextNodeId, nextSegmentId, ref netManager.m_segments.m_buffer[(int)nextSegmentId], ref similarLaneIndexFromLeft, connectOffset, false, true);
						}
					}
				}
				NetInfo.LaneType laneType = _laneTypes & ~NetInfo.LaneType.Pedestrian;
				VehicleInfo.VehicleType vehicleType = _vehicleTypes & ~VehicleInfo.VehicleType.Bicycle;
...
#endif
					ProcessItemPedBicycle(item, nextNodeId, prevSegmentId, ref netManager.m_segments.m_buffer[(int)prevSegmentId], connectOffset2, nextLaneIndex2, nextlaneId2); // ped
				}
			}
			else {
				bool mayTurnAround = (nextNode.m_flags & (NetNode.Flags.End | NetNode.Flags.OneWayOut)) != NetNode.Flags.None;
				bool pedestrianAllowed = (byte)(_laneTypes & NetInfo.LaneType.Pedestrian) != 0;
				...
				if (pedestrianAllowed)
				{
					...
					else if (_vehicleLane != 0u)
					{
						if (_vehicleLane != item.m_laneID)
						{
							pedestrianAllowed = false;
						}
						else {
							nextConnectOffset = _vehicleOffset;
						}
					}
					else if (_stablePath)
					{
						...
						nextConnectOffset = (byte)_pathRandomizer.UInt32(1u, 254u);
					}
				}
				...
				bool explorePrevSegment =
					...
					((_unitType & ExtendedUnitType.RoadVehicle) != ExtendedUnitType.None);
				...
				for (int k = 0; k < 8; k++)
				{
					...
					else if (!nextIsBeautificationNode)
					{
...
#if DEBUGPF
							if (debug)
								logBuf.Add($"Exploring path from {nextSegmentId} to {item.m_position.m_segment}, lane id {item.m_position.m_lane}, {prevRightSimilarLaneIndex} from right: Handling everything that is not a car: {_vehicleTypes}");
#endif
						...
						NetInfo.LaneType drivingEnabledLaneTypes = _laneTypes;
						...
						byte laneIndex = 0;
						_compatibleRightSimilarIndexesMask = 0; // holds a bitmask that indicates which elements in `laneIndexByRightSimilarIndex` are valid for the current run
						...
						while (laneIndex < nextSegmentInfo.m_lanes.Length && curLaneId != 0u)
						{
							...
							if (isIncomingLane && isCompatibleLane)
							{
								...
								{
									_laneIndexes[curLaneI] = laneIndex;
									_laneIds[curLaneI] = curLaneId;
									_laneRightSimilarIndexes[curLaneI] = nextRightSimilarLaneIndex;
									_laneLeftSimilarIndexes[curLaneI] = nextLeftSimilarLaneIndex;
									_laneIndexByRightSimilarIndex[nextRightSimilarLaneIndex] = curLaneI;
									_compatibleRightSimilarIndexesMask |= _pow2Masks[nextRightSimilarLaneIndex];
#if DEBUGPF
									if (debug)
						...
						if (curLaneI > 0)
						{
							...
							if (applyHighwayRulesAtSegment)
							{
								...
								if (nextLeftSimilarIndex >= 0 && nextLeftSimilarIndex < nextCompatibleLaneCount)
								{
									...
									nextLaneI = FindValue(ref _laneLeftSimilarIndexes, nextLeftSimilarIndex, nextCompatibleLaneCount);// Convert.ToInt32(indexByLeftSimilarLaneIndex[nextLeftSimilarIndex]) - 1;
#if DEBUGPF
									if (debug)
									...
									if (nextLeftSimilarIndex < 0)
									{
										if (totalIncomingLanes >= prevSimiliarLaneCount)
										{
											...
											nextLaneI = FindValue(ref _laneRightSimilarIndexes, prevRightSimilarLaneIndex, nextCompatibleLaneCount);
										}
									}
									else {
										if (totalOutgoingLanes >= nextCompatibleLaneCount)
										{
											...
											nextLaneI = FindValue(ref _laneRightSimilarIndexes, 0, nextCompatibleLaneCount);
										}
									}
							...
							else if (nextCompatibleLaneCount == 1)
							{
								...
								for (short nextRightSimilarIndex = minNextRightSimilarIndex; nextRightSimilarIndex <= maxNextRightSimilarIndex; ++nextRightSimilarIndex)
								{
...
#endif
									nextLaneI = FindCompatibleLane(ref _laneIndexByRightSimilarIndex, _compatibleRightSimilarIndexesMask, nextRightSimilarIndex);
									...
									// go to matched lane
									nextLaneIndex = _laneIndexes[nextLaneI];
									nextLaneId = _laneIds[nextLaneI];
						...
						// stock code:
						if (ProcessItemCosts(false, debug, item, nextNodeId, nextSegmentId, ref netManager.m_segments.m_buffer[(int)nextSegmentId], ref similarLaneIndexFromLeft, connectOffset, true, nextIsBeautificationNode))
						{
							mayTurnAround = true;
						}
#if DEBUGPF
...
#endif
					// stock code:
					/*if (ProcessItem(debug, item, nextNodeId, nextSegmentId, ref instance.m_segments.m_buffer[(int)nextSegmentId], ref similarLaneIndexFromLeft, connectOffset, true, enablePedestrian)) {
                        blocked = true;
                    }*/
					//}
...
#endif
				if (!explorePrevSegment && mayTurnAround && (_vehicleTypes & VehicleInfo.VehicleType.Tram) == VehicleInfo.VehicleType.None)
				{
					...
					ProcessItemCosts(false, debug, item, nextNodeId, nextSegmentId, ref netManager.m_segments.m_buffer[(int)nextSegmentId], ref similarLaneIndexFromLeft, connectOffset, true, false);
				}
				...
				if (pedestrianAllowed)
				{
					...
					int nextLaneIndex;
					uint nextLaneId;
					if (netManager.m_segments.m_buffer[(int)nextSegmentId].GetClosestLane((int)item.m_position.m_lane, NetInfo.LaneType.Pedestrian, _vehicleTypes, out nextLaneIndex, out nextLaneId))
					{
#if DEBUGPF
						if (debug)
...
#endif
						ProcessItemPedBicycle(item, nextNodeId, nextSegmentId, ref netManager.m_segments.m_buffer[(int)nextSegmentId], nextConnectOffset, nextLaneIndex, nextLaneId); // ped
					}
				}
			}
...
#endif
					ProcessItemPublicTransport(item, nextNodeId, targetDisabled, nextSegment, ref netManager.m_segments.m_buffer[(int)nextSegment], nextNode.m_lane, nextNode.m_laneOffset, connectOffset);
				}
			}
			...
			if (debug) {
				foreach (String toLog in logBuf) {
					Log._Debug($"Pathfinder ({_pathFindIndex}) for unit {unitId}: " + toLog);
				}
			}
#endif
#if DEBUGPF2
		...
		{
			short nextLaneI = -1;
			for (byte i = 0; i < _pow2Masks.Length; ++i)
			{
				if ((validMask & _pow2Masks[i]) == 0)
					continue;
		...
		private void HandleLaneMergesAndSplits(short prevRightSimilarLaneIndex, short nextCompatibleLaneCount, short prevSimilarLaneCount, out short minNextRightSimilarLaneIndex, out short maxNextRightSimilarLaneIndex)
		{
			...
			{
#if DEBUGPF
				Log.Error($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: Erroneous calculation in HandleMergeAndSplits detected!");
#endif
		...
		private void ProcessItemPublicTransport(BufferItem item, ushort targetNodeId, bool targetDisabled, ushort nextSegmentId, ref NetSegment nextSegment, uint nextLane, byte offset, byte connectOffset)
		{
			...
			if ((int)item.m_position.m_lane < prevSegmentInfo.m_lanes.Length)
			{
				...
				prevMaxSpeed = this.GetFeatures().SpeedLimits.GetLaneSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, lane2, _unitType);
				// prevMaxSpeed = GetLaneSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, lane2);
				// prevMaxSpeed = TAMSpeedLimitManager.instance.GetLockFreeGameSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, ref lane2); // NON-STOCK CODE
				laneType = lane2.m_laneType;
				if ((byte)(laneType & (NetInfo.LaneType.Vehicle | NetInfo.LaneType.TransportVehicle)) != 0)
				{
					laneType = (NetInfo.LaneType.Vehicle | NetInfo.LaneType.TransportVehicle);
				}
				prevSpeed = CalculateLaneSpeed(prevMaxSpeed, connectOffset, item.m_position.m_offset, ref instance.m_segments.m_buffer[(int)item.m_position.m_segment], lane2); // NON-STOCK CODE
			}
			...
			float comparisonValue = item.m_comparisonValue + offsetLength / (prevSpeed * _maxLength);
			...
			while (laneIndex < nextNumLanes && curLaneId != 0u)
			{
				...
				if (nextLane == curLaneId)
				{
					...
					if (nextLaneInfo.CheckType(_laneTypes, _vehicleTypes))
					{
						...
						float nextMaxSpeed = this.GetFeatures().SpeedLimits.GetLaneSpeedLimit(nextSegmentId, laneIndex, curLaneId, nextLaneInfo, _unitType);  // NON-STOCK CODE
																																							// float nextMaxSpeed = GetLaneSpeedLimit(nextSegmentId, laneIndex, curLaneId, nextLaneInfo);  // NON-STOCK CODE
																																							// float nextMaxSpeed = TAMSpeedLimitManager.instance.GetLockFreeGameSpeedLimit(nextSegmentId, laneIndex, curLaneId, ref lane3); // NON-STOCK CODE
						if (nextLaneInfo.m_laneType != NetInfo.LaneType.Pedestrian || nextItem.m_methodDistance < 1000f)
						{
							nextItem.m_comparisonValue = comparisonValue + distance / ((prevMaxSpeed + nextMaxSpeed) * 0.5f * _maxLength); // NON-STOCK CODE
							...
							if (nextLane == _startLaneA)
							{
								if (((byte)(nextItem.m_direction & NetInfo.Direction.Forward) == 0 || nextItem.m_position.m_offset < _startOffsetA) && ((byte)(nextItem.m_direction & NetInfo.Direction.Backward) == 0 || nextItem.m_position.m_offset > _startOffsetA))
								{
									return;
								}
								float nextSpeed = CalculateLaneSpeed(nextMaxSpeed, _startOffsetA, nextItem.m_position.m_offset, ref nextSegment, nextLaneInfo); // NON-STOCK CODE
								float nextOffsetDistance = (float)Mathf.Abs((int)(nextItem.m_position.m_offset - _startOffsetA)) * 0.003921569f;
								nextItem.m_comparisonValue += nextOffsetDistance * nextSegment.m_averageLength / (nextSpeed * _maxLength);
							}
							if (nextLane == _startLaneB)
							{
								if (((byte)(nextItem.m_direction & NetInfo.Direction.Forward) == 0 || nextItem.m_position.m_offset < _startOffsetB) && ((byte)(nextItem.m_direction & NetInfo.Direction.Backward) == 0 || nextItem.m_position.m_offset > _startOffsetB))
								{
									return;
								}
								float nextSpeed = CalculateLaneSpeed(nextMaxSpeed, _startOffsetB, nextItem.m_position.m_offset, ref nextSegment, nextLaneInfo); // NON-STOCK CODE
								float nextOffsetDistance = (float)Mathf.Abs((int)(nextItem.m_position.m_offset - _startOffsetB)) * 0.003921569f;
								nextItem.m_comparisonValue += nextOffsetDistance * nextSegment.m_averageLength / (nextSpeed * _maxLength);
							}
		...
		private bool ProcessItemCosts(bool allowCustomLaneChanging, bool debug, BufferItem item, ushort targetNodeId, ushort nextSegmentId, ref NetSegment nextSegment, ref int laneIndexFromLeft, byte connectOffset, bool enableVehicle, bool enablePedestrian, int? forceLaneIndex, uint? forceLaneId, out bool foundForced)
		{
#if DEBUGPF
			/*if (Options.disableSomething1)
				Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: ProcessItemSub item {item.m_position.m_segment} {item.m_position.m_lane}, targetNodeId {targetNodeId}");*/
#endif
			...
			if ((int)item.m_position.m_lane < prevSegmentInfo.m_lanes.Length)
			{
				...
				prevMaxSpeed = this.GetFeatures().SpeedLimits.GetLaneSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, prevLaneInfo, _unitType);  // NON-STOCK CODE
				prevLaneSpeed = CalculateLaneSpeed(prevMaxSpeed, connectOffset, item.m_position.m_offset, ref instance.m_segments.m_buffer[(int)item.m_position.m_segment], prevLaneInfo); // NON-STOCK CODE
																																														   // NON-STOCK CODE START //
			...
			if (!useAdvancedAI && !_stablePath)
			{ // NON-STOCK CODE
			  // CO randomization. Only randomizes over segments, not over lanes.

				Randomizer randomizer = new Randomizer(_pathFindIndex << 16 | (uint)item.m_position.m_segment);
				prevCost *= (float)(randomizer.Int32(900, 1000 + (int)(instance.m_segments.m_buffer[(int)item.m_position.m_segment].m_trafficDensity * 10)) + _pathRandomizer.Int32(20u)) * 0.001f;
			}
			if (!useAdvancedAI)
			{
				// stock code check for vehicle ban policies

				if (_isHeavyVehicle && (instance.m_segments.m_buffer[(int)item.m_position.m_segment].m_flags & NetSegment.Flags.HeavyBan) != NetSegment.Flags.None)
				{
					// heavy vehicle ban
					prevCost *= 10f;
				}
			...
			if (_transportVehicle && prevLaneType == NetInfo.LaneType.TransportVehicle)
			{
			...
			if ((_isHeavyVehicle && (nextSegment.m_flags & NetSegment.Flags.HeavyBan) != NetSegment.Flags.None) ||
				(prevLaneType == NetInfo.LaneType.Vehicle && (prevVehicleType & _vehicleTypes) == VehicleInfo.VehicleType.Car && (instance.m_segments.m_buffer[(int)item.m_position.m_segment].m_flags & NetSegment.Flags.CarBan) != NetSegment.Flags.None))
			{
				...
				if (Options.disableSomething1 && debug) {
					Log._Debug($"Vehicle {_unitType} should not use lane {item.m_position.m_lane} @ seg. {item.m_position.m_segment}, null? {lane == null}");
				}
#endif
				avoidLane = true;
			}
			...
			if (!this.GetFeatures().Restrictions.CanUseLane(item.m_position.m_segment, prevSegmentInfo, item.m_position.m_lane, item.m_laneID, prevLaneInfo, _unitType)) {
			//if (false) {
#if DEBUGPF
				if (Options.disableSomething1 && debug) {
			...
			float prevComparisonPlusOffsetCostOverSpeed = item.m_comparisonValue + prevOffsetCost / (prevLaneSpeed * _maxLength);
			...
			NetInfo.LaneType allowedLaneTypes = _laneTypes;
			VehicleInfo.VehicleType allowedVehicleTypes = _vehicleTypes;
			...
			while (laneIndex < nextNumLanes && curLaneId != 0u)
			{
				...
				if ((byte)(nextLane.m_finalDirection & nextFinalDir) != 0 &&
					this.GetFeatures().LaneRouting.CanLanesConnect(targetNodeId, nextSegmentId, (byte)laneIndex, curLaneId, item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, _unitType))
				{ // NON-STOCK CODE
				  // lane direction is compatible
							...
							(nextSegmentId != item.m_position.m_segment || laneIndex != (int)item.m_position.m_lane))
					{
						...
						// NON-STOCK CODE START //
						float nextMaxSpeed = this.GetFeatures().SpeedLimits.GetLaneSpeedLimit(nextSegmentId, laneIndex, curLaneId, nextLane, _unitType);
						//float nextMaxSpeed = GetLaneSpeedLimit(nextSegmentId, laneIndex, curLaneId, nextLane);
						bool addCustomTrafficCosts = useAdvancedAI &&
							curLaneId != _startLaneA &&
							curLaneId != _startLaneB &&
						...
						float distanceOverMeanMaxSpeed = distanceOnBezier / ((prevMaxSpeed + nextMaxSpeed) * 0.5f * _maxLength);
						BufferItem nextItem;
						// NON-STOCK CODE START //
						if (prevIsJunction)
							nextItem.m_numSegmentsToJunction = 0;
						...
						if (nextLane.m_laneType != NetInfo.LaneType.Pedestrian || nextItem.m_methodDistance < 1000f)
						{
							...
							if (!addCustomTrafficCosts)
							{
								...
								else if (avoidLane && (_unitType == ExtendedUnitType.Unknown || (_unitType & (ExtendedUnitType.CargoTruck | ExtendedUnitType.PassengerCar)) != ExtendedUnitType.None))
								{
							...
							if (curLaneId == _startLaneA)
							{
								...
								float nextLaneSpeed = CalculateLaneSpeed(nextMaxSpeed, _startOffsetA, nextItem.m_position.m_offset, ref nextSegment, nextLane); // NON-STOCK CODE
								float nextOffset = (float)Mathf.Abs((int)(nextItem.m_position.m_offset - _startOffsetA)) * 0.003921569f;
								nextItem.m_comparisonValue += nextOffset * nextSegment.m_averageLength / (nextLaneSpeed * _maxLength);
							}
							if (curLaneId == _startLaneB)
							{
								if (((byte)(nextItem.m_direction & NetInfo.Direction.Forward) == 0 || nextItem.m_position.m_offset < _startOffsetB) && ((byte)(nextItem.m_direction & NetInfo.Direction.Backward) == 0 || nextItem.m_position.m_offset > _startOffsetB))
								{
									curLaneId = instance.m_lanes.m_buffer[(int)((UIntPtr)curLaneId)].m_nextLane;
								...
								float nextLaneSpeed = CalculateLaneSpeed(nextMaxSpeed, _startOffsetB, nextItem.m_position.m_offset, ref nextSegment, nextLane); // NON-STOCK CODE
								float nextOffset = (float)Mathf.Abs((int)(nextItem.m_position.m_offset - _startOffsetB)) * 0.003921569f;
								nextItem.m_comparisonValue += nextOffset * nextSegment.m_averageLength / (nextLaneSpeed * _maxLength);
							}

							if (!_ignoreBlocked && (nextSegment.m_flags & NetSegment.Flags.Blocked) != NetSegment.Flags.None && (byte)(nextLane.m_laneType & (NetInfo.LaneType.Vehicle | NetInfo.LaneType.TransportVehicle)) != 0)
							{
								// NON-STOCK CODE START //
								if (addCustomTrafficCosts)
							...
							if ((byte)(nextLane.m_laneType & prevLaneType) != 0 && nextLane.m_vehicleType == prevVehicleType)
							{
								...
								if (!addCustomTrafficCosts)
								{
									...
									{
										nextItem.m_comparisonValue += Mathf.Max(1f, distanceOnBezier * 3f - 3f) / ((prevMaxSpeed + nextMaxSpeed) * 0.5f * _maxLength);
									}

									// cars should not be on public transport lanes (when Advanced AI is activated, this is handled by the flag `strictlyAvoidLane`)
									if (!_transportVehicle && nextLane.m_laneType == NetInfo.LaneType.TransportVehicle)
									{
										nextItem.m_comparisonValue += 20f / ((prevMaxSpeed + nextMaxSpeed) * 0.5f * _maxLength);
									}
									// NON-STOCK CODE START //
								}
								// NON-STOCK CODE END //
							...
							if (addCustomTrafficCosts)
							{
								...
								*/
								...
								if ((!isMiddle && nextSegmentId == item.m_position.m_segment) || // u-turns
									(_unitType != ExtendedUnitType.Emergency && // emergency vehicles may do everything
								...
								distanceOverMeanMaxSpeed = (metric * distanceOnBezier) / _maxLength;
								...
								if (distanceOverMeanMaxSpeed < 0f)
								{
									// should never happen
#if DEBUG
									Log.Error($"THREAD #{Thread.CurrentThread.ManagedThreadId}, PF {_pathFindIndex}: distanceOverMeanMaxSpeed < 0! seg. {nextSegmentId}, lane {laneIndex}, off {nextItem.m_position.m_offset} -> {item.m_position.m_segment}, lane {item.m_position.m_lane}, off {item.m_position.m_offset}. distanceOverMeanMaxSpeed={distanceOverMeanMaxSpeed}, nextSpeed={nextSpeed}, prevSpeed={prevSpeed}");
#endif
								...
								if (costDebug) {
									foreach (String toLog in logBuf) {
										Log._Debug($"Pathfinder ({_pathFindIndex}): " + toLog);
									}
									logBuf.Clear();
								}
#endif
...
#if DEBUGPF
									if (debug)
										Log._Debug($"Pathfinder ({_pathFindIndex}): comparisonValue is >1, NaN or Infinity: {nextItem.m_comparisonValue}. seg. {nextSegmentId}, lane {laneIndex}, off {nextItem.m_position.m_offset} -> {item.m_position.m_segment}, lane {item.m_position.m_lane}, off {item.m_position.m_offset}.");
#endif
#if DEBUG
									//Log.Error($"THREAD #{Thread.CurrentThread.ManagedThreadId}, PF {_pathFindIndex}: Comparison value > 1, NaN or infinity! distanceOverMeanMaxSpeed={distanceOverMeanMaxSpeed}, nextSpeed={nextSpeed}, prevSpeed={prevSpeed}");
#endif
									addItem = false;
								}
#if DEBUGPF
								if (debug) {
									//Log.Message($">> PF {_pathFindIndex} -- seg {item2.m_position.m_segment}, lane {item2.m_position.m_lane} (idx {item2.m_laneID}), off {item2.m_position.m_offset}, cost {item2.m_comparisonValue}, totalCost {totalCost} = traffic={trafficCost}, junction={junctionCost}, lane={laneChangeCost}");
								}
#endif
							}
...
#if DEBUGPF
								if (debug)
									Log._Debug($">> PF {_pathFindIndex} -- Adding item: seg {nextItem.m_position.m_segment}, lane {nextItem.m_position.m_lane} (idx {nextItem.m_laneID}), off {nextItem.m_position.m_offset} -> seg {item.m_position.m_segment}, lane {item.m_position.m_lane} (idx {item.m_laneID}), off {item.m_position.m_offset}, cost {nextItem.m_comparisonValue}, previous cost {item.m_comparisonValue}, methodDist {nextItem.m_methodDistance}");
#endif
...
#if DEBUGPF
								if (debug)
									Log._Debug($">> PF {_pathFindIndex} -- NOT adding item");
#endif
							}
							// NON-STOCK CODE END //
						}
		...
		private void ProcessItemPedBicycle(BufferItem item, ushort targetNodeId, ushort nextSegmentId, ref NetSegment nextSegment, byte connectOffset, int laneIndex, uint lane)
		{
			...
			{
				NetInfo.Lane prevLane = prevSegmentInfo.m_lanes[(int)item.m_position.m_lane];
				prevMaxSpeed = this.GetFeatures().SpeedLimits.GetLaneSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, prevLane, _unitType);  // NON-STOCK CODE
																																									   // prevMaxSpeed = GetLaneSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, prevLane);  // NON-STOCK CODE
																																									   // TAMSpeedLimitManager.instance.GetLockFreeGameSpeedLimit(item.m_position.m_segment, item.m_position.m_lane, item.m_laneID, ref lane2); // NON-STOCK CODE
				...
				prevSpeed = CalculateLaneSpeed(prevMaxSpeed, connectOffset, item.m_position.m_offset, ref netManager.m_segments.m_buffer[(int)item.m_position.m_segment], prevLane); // NON-STOCK CODE
			}
			float prevCost = netManager.m_segments.m_buffer[(int)item.m_position.m_segment].m_averageLength;
			// NON-STOCK CODE START
			if (_unitType == ExtendedUnitType.Bicycle)
			{
			...
			if (laneIndex < num)
			{
				...
				if ((byte)(nextLane.m_laneType & laneType) == 0)
				{
					...
					{
						comparisonValue += 100f / (0.25f * _maxLength);
					}
					nextItem.m_methodDistance = methodDistance + distance;
				}
				float nextMaxSpeed = this.GetFeatures().SpeedLimits.GetLaneSpeedLimit(nextSegmentId, (uint)laneIndex, lane, nextLane, _unitType); // NON-STOCK CODE
																																				//float nextMaxSpeed = GetLaneSpeedLimit(nextSegmentId, (uint)laneIndex, lane, nextLane); // NON-STOCK CODE
				if (nextLane.m_laneType != NetInfo.LaneType.Pedestrian || nextItem.m_methodDistance < 1000f)
				{
					nextItem.m_comparisonValue = comparisonValue + distance / ((prevMaxSpeed + nextMaxSpeed) * 0.25f * _maxLength);
					if ((nextSegment.m_flags & NetSegment.Flags.Invert) != NetSegment.Flags.None)
					{
					...
					if (lane == _startLaneA)
					{
						...
						float nextSpeed = CalculateLaneSpeed(nextMaxSpeed, _startOffsetA, nextItem.m_position.m_offset, ref nextSegment, nextLane); // NON-STOCK CODE
						float nextOffset = (float)Mathf.Abs((int)(nextItem.m_position.m_offset - _startOffsetA)) * 0.003921569f;
						nextItem.m_comparisonValue += nextOffset * nextSegment.m_averageLength / (nextSpeed * _maxLength);
					}
					if (lane == _startLaneB)
					{
						if (((byte)(nextItem.m_direction & NetInfo.Direction.Forward) == 0 || nextItem.m_position.m_offset < _startOffsetB) && ((byte)(nextItem.m_direction & NetInfo.Direction.Backward) == 0 || nextItem.m_position.m_offset > _startOffsetB))
						...
						float nextSpeed = CalculateLaneSpeed(nextMaxSpeed, _startOffsetB, nextItem.m_position.m_offset, ref nextSegment, nextLane); // NON-STOCK CODE
						float nextOffset = (float)Mathf.Abs((int)(nextItem.m_position.m_offset - _startOffsetB)) * 0.003921569f;
						nextItem.m_comparisonValue += nextOffset * nextSegment.m_averageLength / (nextSpeed * _maxLength);
					}
		...
		private void PathFindThread()
		{
			while (true)
			{
				...
				try
				{
					...
					_calculating = _queueFirst;
					_queueFirst = _pathUnits.m_buffer[(int)((UIntPtr)_calculating)].m_nextPathUnit;
					if (_queueFirst == 0u)
					{
						_queueLast = 0u;
...
#endif
					_pathUnits.m_buffer[(int)((UIntPtr)_calculating)].m_nextPathUnit = 0u;
					_pathUnits.m_buffer[(int)((UIntPtr)_calculating)].m_pathFindFlags = (byte)((_pathUnits.m_buffer[(int)((UIntPtr)_calculating)].m_pathFindFlags & -2) | 2);
				}
				catch (Exception e)
				{
...
#if DEBUGPF
					/*if (m_queuedPathFindCount > 100 && Options.disableSomething1)
						Log._Debug($"THREAD #{Thread.CurrentThread.ManagedThreadId} PF {_pathFindIndex}: Calling PathFindImplementation now. m_calculating={m_calculating}");*/
#endif
					PathFindImplementation(_calculating, ref _pathUnits.m_buffer[(int)((UIntPtr)_calculating)]);
				}
				catch (Exception ex)
				{
					Log.Error($"THREAD #{Thread.CurrentThread.ManagedThreadId} Path find error: " + ex.ToString());
					//UIView.ForwardException(ex);
					_pathUnits.m_buffer[(int)_calculating].m_pathFindFlags |= PathUnit.FLAG_FAILED;
				}
				finally
				{
				...
				{
					Monitor.Enter(_queueLock);
					_pathUnits.m_buffer[(int)((UIntPtr)_calculating)].m_pathFindFlags = (byte)(_pathUnits.m_buffer[(int)((UIntPtr)_calculating)].m_pathFindFlags & -3);
					Singleton<PathManager>.instance.ReleasePath(_calculating);
					_calculating = 0u;
					Monitor.Pulse(_queueLock);
				}
				catch (Exception e)
				{
					Log.Error("CustomPathFind.PathFindThread Error (3): " + e);
				}
				finally
				{
					Monitor.Exit(_queueLock);
