	...
	public class SpeedLimitsTool : SubTool {
		...
		private void ShowSigns(bool viewOnly) {
			...
			for (int i = 1; i < segments.m_size; ++i) {
				...
				// draw speed limits
				if (TrafficManagerTool.GetToolMode() != TrafficManagerToolMode.VehicleRestrictions || i != SelectedSegmentId) { // no speed limit overlay on selected segment when in vehicle restrictions mode
