	...
	public class TimedTrafficLightsTool : SubTool {
		...
		public override void OnClickOverlay() {
			...
			switch (TrafficManagerTool.GetToolMode()) {
				case TrafficManagerToolMode.TimedLightsSelectNode:
				case TrafficManagerToolMode.TimedLightsShowLights:
					if (TrafficManagerTool.GetToolMode() == TrafficManagerToolMode.TimedLightsShowLights) {
						TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsSelectNode);
						ClearSelectedNodes();
					}
					...
					if (timedSim == null || !timedSim.IsTimedLight()) {
						...
						if (SelectedNodeIndexes.Count == 0) {
							...
							if (timedLight != null) {
								...
								TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsShowLights);
							}
						} else {
					...
					break;
				case TrafficManagerToolMode.TimedLightsAddNode:
					if (SelectedNodeIndexes.Count <= 0) {
						TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsSelectNode);
						return;
					}
					...
					TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsShowLights);
					break;
				case TrafficManagerToolMode.TimedLightsRemoveNode:
					if (SelectedNodeIndexes.Count <= 0) {
						TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsSelectNode);
						return;
					}
		...
		public override void RenderOverlay(RenderManager.CameraInfo cameraInfo) {
			bool onlySelected = TrafficManagerTool.GetToolMode() == TrafficManagerToolMode.TimedLightsRemoveNode;
		...
		private void _guiTimedControlPanel(int num) {
			...
			if (TrafficManagerTool.GetToolMode() == TrafficManagerToolMode.TimedLightsAddNode || TrafficManagerTool.GetToolMode() == TrafficManagerToolMode.TimedLightsRemoveNode) {
...
//								Log._Debug("Starting traffic light @ " + sim.TimedLight.NodeId);
			...
			if (GUILayout.Button(Translation.GetString("Add_junction_to_timed_light"))) {
				TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsAddNode);
			}

			if (SelectedNodeIndexes.Count > 1) {
				if (GUILayout.Button(Translation.GetString("Remove_junction_from_timed_light"))) {
					TrafficManagerTool.SetToolMode(TrafficManagerToolMode.TimedLightsRemoveNode);
				}
			}
		...
		private void _guiTimedTrafficLights() {
			...
			foreach (var nodeId in SelectedNodeIndexes) {
				...
				for (var i = 0; i < 8; i++) {
					...
					int lightOffset = -1;
					foreach (TMVehicleType vehicleType in liveSegmentLights.VehicleTypes) {
						++lightOffset;
		...
		public override void ShowIcons() {
			if (!TMDataManager.Options.timedLightsOverlay &&
				...
				TrafficManagerTool.GetToolMode() != TrafficManagerToolMode.TimedLightsShowLights)
				return;
