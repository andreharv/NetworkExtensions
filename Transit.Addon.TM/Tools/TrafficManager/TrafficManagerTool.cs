...
namespace Transit.Addon.TM.Tools {
	...
	public class TrafficManagerTool : DefaultTool {
		private static TrafficManagerToolMode _toolMode;
		...
		protected override void Awake() {
			...
			if (!initDone) {
				...
				subTools[(int)TrafficManagerToolMode.SwitchTrafficLight] = new ToggleTrafficLightsTool(this);
				subTools[(int)TrafficManagerToolMode.AddPrioritySigns] = new PrioritySignsTool(this);
				subTools[(int)TrafficManagerToolMode.ManualSwitch] = new ManualTrafficLightsTool(this);
				SubTool timedLightsTool = new TimedTrafficLightsTool(this);
				subTools[(int)TrafficManagerToolMode.TimedLightsAddNode] = timedLightsTool;
				subTools[(int)TrafficManagerToolMode.TimedLightsRemoveNode] = timedLightsTool;
				subTools[(int)TrafficManagerToolMode.TimedLightsSelectNode] = timedLightsTool;
		...
		public static TrafficManagerToolMode GetToolMode() {
			return _toolMode;
		}

		public static void SetToolMode(TrafficManagerToolMode mode) {
			...
			if (toolModeChanged && activeSubTool != null) {
				if ((_toolMode == TrafficManagerToolMode.TimedLightsSelectNode || _toolMode == TrafficManagerToolMode.TimedLightsShowLights || _toolMode == TrafficManagerToolMode.TimedLightsAddNode || _toolMode == TrafficManagerToolMode.TimedLightsRemoveNode)) { // TODO refactor to SubToolMode
					if (mode != TrafficManagerToolMode.TimedLightsSelectNode && mode != TrafficManagerToolMode.TimedLightsShowLights && mode != TrafficManagerToolMode.TimedLightsAddNode && mode != TrafficManagerToolMode.TimedLightsRemoveNode)
		...
		public override void SimulationStep() {
			...
			if (GetToolMode() == TrafficManagerToolMode.None) {
				ToolCursor = null;
			} else {
				bool elementsHovered = determineHoveredElements();
...
#if DEBUG
				labelStr += ", flags: " + ((NetLane.Flags)Singleton<NetManager>.instance.m_lanes.m_buffer[curLaneId].m_flags).ToString() + ", limit: " + TAMSpeedLimitManager.instance.GetCustomSpeedLimit(curLaneId) + " km/h, dir: " + laneInfo.m_direction + ", final: " + laneInfo.m_finalDirection + ", pos: " + String.Format("{0:0.##}", laneInfo.m_position) + ", sim. idx: " + laneInfo.m_similarLaneIndex + " for " + laneInfo.m_vehicleType;
#endif
