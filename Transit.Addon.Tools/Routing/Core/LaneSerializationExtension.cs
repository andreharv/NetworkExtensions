...
namespace Transit.Addon.Tools.Core
{
    public class LaneSerializationExtension : SerializableDataExtensionBase
    {
        ...
        public override void OnLoadData()
        {
            ...
            try
            {
                LanesManager.instance.sm_lanes = (Lane[]) binaryFormatter.Deserialize(memStream);
                    
                FastList<ushort> nodesList = new FastList<ushort>();
                foreach (Lane lane in LanesManager.instance.sm_lanes)
                {
                    if (lane == null)
                        continue;
        ...
        public override void OnSaveData()
        {
            ...
            try
            {
                binaryFormatter.Serialize(memStream, LanesManager.instance.sm_lanes);
