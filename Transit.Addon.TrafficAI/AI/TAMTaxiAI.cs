...
namespace Transit.Addon.TrafficAI
{
    public class CustomTaxiAI : CarAI
    {
        ...
        protected override bool StartPathFind(ushort vehicleID, ref Vehicle vehicleData, Vector3 startPos, Vector3 endPos, bool startBothWays, bool endBothWays)
        {
            ...
            ushort passengerInstance = GetPassengerInstance(vehicleID, ref vehicleData);
            ...
            CitizenInfo info2 = instance.m_instances.m_buffer[passengerInstance].Info;
            NetInfo.LaneType laneType = NetInfo.LaneType.Vehicle | NetInfo.LaneType.Pedestrian | NetInfo.LaneType.TransportVehicle;
            VehicleInfo.VehicleType vehicleType = m_info.m_vehicleType | (VehicleInfo.VehicleType)ExtendedVehicleType.Taxi;
            ...
            if (PathManager.FindPathPosition(startPos, ItemClass.Service.Road, NetInfo.LaneType.Vehicle | NetInfo.LaneType.TransportVehicle, info.m_vehicleType, allowUnderground,
                false, 32f, out startPosA, out startPosB, out num, out num2) &&
                info2.m_citizenAI.FindPathPosition(passengerInstance, ref instance.m_instances.m_buffer[passengerInstance], endPos, laneType, vehicleType, false, out endPosA))
            {
                ...
                uint path;
                if (PathManager.instance.CreatePath(out path, ref instance2.m_randomizer, instance2.m_currentBuildIndex, startPosA, startPosB, endPosA, endPosB, laneType, vehicleType, 20000f))
                {
        ...
        private static ushort GetPassengerInstance(ushort vehicleID, ref Vehicle vehicleData)
        {
