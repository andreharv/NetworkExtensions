...
{
    public class TAMPathManager : SimulationManagerBase<TAMPathManager, PathProperties>, ISimulationManager
    {
        public class Data : IDataContainer
        {
            public void Serialize(DataSerializer s)
            {
                ...
                TAMPathManager instance = Singleton<TAMPathManager>.instance;
            ...
            for (uint num2 = 0u; num2 < this.m_pathUnits.m_size; num2 += 1u)
            {
                if ((this.m_pathUnits.m_buffer[(int)((UIntPtr)num2)].m_pathFindFlags & 4) != 0)
                {
                    ...
                    if (positionCount >= 1 && num++ == this.m_renderPathGizmo)
                    {
                        ...
                        {
                            Vector3 vector = TAMPathManager.CalculatePosition(pathPos);
                            Gizmos.DrawSphere(vector, 8f);
                            for (int i = 1; i < positionCount; i++)
                            {
                                if (this.m_pathUnits.m_buffer[(int)((UIntPtr)num2)].GetPosition(i, out pathPos))
                                {
                                    Vector3 vector2 = TAMPathManager.CalculatePosition(pathPos);
        ...
        public static bool FindPathPosition(Vector3 position, ItemClass.Service service, NetInfo.LaneType laneType, VehicleInfo.VehicleType vehicleType, bool allowUnderground, bool requireConnect, float maxDistance, out PathUnit.Position pathPos)
        {
            ...
            float num;
            float num2;
            return TAMPathManager.FindPathPosition(position, service, laneType, vehicleType, allowUnderground, requireConnect, maxDistance, out pathPos, out position2, out num, out num2);
        }
        ...
        {
            base.GetData(data);
            data.Add(new TAMPathManager.Data());
        }
