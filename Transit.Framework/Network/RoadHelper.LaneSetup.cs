    ...
    public static partial class RoadHelper
    {
        ...
        public static IEnumerable<NetInfo.Lane> SetupPedestrianLanes(this NetInfo rdInfo, NetInfoVersion version, LanesConfiguration config)
        {
            var pedestrianLanes = rdInfo.m_lanes
                ...
                .ToArray();
            var hasOffsetPavement = rdInfo.m_segments.Any(s => s.m_mesh.name == "Ground_Pavement");
            if (version.IsGroundDecorated() && hasOffsetPavement )
            {
                config.PedPropOffsetX = -1;
            }
            foreach (var pedLane in pedestrianLanes)
            {
                ...
                pedLane.m_width = rdInfo.m_pavementWidth;
                if (version == NetInfoVersion.Slope || version == NetInfoVersion.Tunnel || (hasOffsetPavement && (version == NetInfoVersion.Ground || version == NetInfoVersion.GroundGrass || version == NetInfoVersion.GroundTrees)))
                {
                    pedLane.m_width -= 3;
                }
                else
